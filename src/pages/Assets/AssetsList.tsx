...
import { HeadingWithPotion2 } from 'components/Heading/HeadingWithPotion2'
...
import { Ether, XBRDX } from 'constants/tokens'
...
export const AssetsListPage = () => {
  ...
  const ethBalance = useCurrencyBalance(account ?? undefined, Ether.onChain(chainId || ChainId.MAINNET) as Currency)
  ...
  const tokensWithBalance = useMemo(() => {
    return [ethBalance, ...Object.values(tokenBalances)]
      ...
      .filter((a) => {
        ...
        const xBrdx = XBRDX[chainId]
        if (!xBrdx) {
          return true
        }
        return xBrdx.address !== a?.currency.wrapped.address
      })
      .filter(isTruthy)
