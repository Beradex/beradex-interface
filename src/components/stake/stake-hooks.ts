...
import { ETHER, tTOKEN, XDIFF } from 'constants/tokens'
...
export function useEarnedDiff(xDiffBalance?: CurrencyAmount<Currency>) {
  ...
  const totalDiffusionToDistribute = useTokenBalance(
    ...
    chainId ? tTOKEN[chainId] : undefined
  )
  const totalXDiff = useTotalSupply(chainId ? tTOKEN[chainId] : undefined)
...
export function useStakingAPY() {
  ...
  const yearlyEmission = chainId
    ? CurrencyAmount.fromRawAmount(tTOKEN[chainId], YEARLY_EMITTED_DIFF).multiply(JSBI.BigInt(10 ** 18))
    : undefined

  console.log('yearlyEmission', yearlyEmission?.toSignificant())

  const totalXDiff = useTotalSupply(chainId ? XDIFF[chainId] : undefined)
