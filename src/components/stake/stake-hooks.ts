...
const DAILY_EMITTED_DIFF = JSBI.BigInt(62500) // beradex: Change Emissions
...
export function useEarnedDiff(xDiffBalance?: CurrencyAmount<Currency>) {
  ...
  const totalXDiff = useTotalSupply(chainId ? BRDX[chainId] : undefined)
...
export function useStakingAPY() {
  ...
  const yearlyEmission = chainId
    ? CurrencyAmount.fromRawAmount(BRDX[chainId], YEARLY_EMITTED_DIFF).multiply(JSBI.BigInt(10 ** 18))
    : undefined

  console.log('yearlyEmission', yearlyEmission?.toSignificant())

  const totalXDiff = useTotalSupply(chainId ? XBRDX[chainId] : undefined)
