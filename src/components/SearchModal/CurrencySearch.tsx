...
export function CurrencySearch({
  ...
  const filteredSortedTokensWithETH: Currency[] = useMemo(() => {
    ...
    if (['', 'b', 'be', 'bera', 'br', 'brd', 'brdx', 't', 'tu', 'tusd'].includes(s)) {
      return [Ether.onChain(chainId || ChainId.MAINNET), ...filteredSortedTokens]
    }
  ...
  const handleEnter = useCallback(
    (e: KeyboardEvent<HTMLInputElement>) => {
      if (e.key === 'Enter') {
        const s = debouncedQuery.toLowerCase().trim()
        if (s === 'eth' && chainId) {
          handleCurrencySelect(Ether.onChain(chainId))
