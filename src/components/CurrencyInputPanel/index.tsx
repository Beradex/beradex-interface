...
const Container = styled.div<{ hideInput: boolean }>`
  ...
  :hover {
    ...
    box-shadow: 0 0 5px rgba(205, 113, 0, 0.15), 0 0 10px rgba(98, 56, 0, 0.3);
  }
`
...
const CurrencySelect = styled(ButtonGray)<{ selected: boolean; hideInput?: boolean }>`
  ...
  border-radius: 10px;
  box-shadow: ${({ selected }) => (selected ? 'none' : '0px 6px 10px rgba(141, 71, 0, 0.075)')};
  box-shadow: 0px 6px 10px rgba(141, 71, 0, 0.075);
  outline: none;
  cursor: pointer;
  user-select: none;
  border: 1px solid ${({ selected, theme }) => (!selected ? '#965c15' ?? 'transparent' : theme.primaryTransparent)};
